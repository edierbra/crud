...
class Usuario extends Conexion
{
  ...
  public function login($user, $clave)
  {
    ...
    $consulta = 'select id_usuario_pk, nombre, cargo, edad, email from usuarios where email="' . $user . '" and clave= MD5("' . $clave . '")';
    /*
    ...
    if ($verificar_usuario > 0) {
      ...
      $_SESSION['id']     = $user['id_usuario_pk'];
  ...
  public function registroUsuario($name, $email, $clave, $edad)
  {
    parent::conectar();
    ...
    $clave = parent::filtrar($clave);
    $edad = parent::filtrar($edad);


    // validar que el correo no exito
    $verificarCorreo = parent::verificarRegistros('select id_usuario_pk, edad, email from usuarios where email="' . $email . '" ');
    ...
    if ($verificarCorreo > 0) {
      ...
      parent::query('insert into usuarios(nombre, edad, email, clave, cargo) values("' . $name . '", "' . $edad . '", "' . $email . '", MD5("' . $clave . '"), 1)');
      ...
      $conexion->conectar();
      // Consulta SQL
      $sql = "SELECT id_usuario_pk FROM usuarios ORDER BY id_usuario_pk DESC LIMIT 1";
      $result = $conexion->query($sql);

      // Verificar si hay resultados
      if ($result->num_rows > 0) {
        // Obtener el último id_usuario_pk
        $row = $result->fetch_assoc();
        $ultimoIdUsuario = $row["id_usuario_pk"];
      }
